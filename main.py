...
def run_module_2(statements):
    """Runs claim extraction on a list of statements from Module 1."""
    print("Running Claim Extraction on each statement...")
    extracted_claims = []
    for stmt in statements:
        # The new function only takes `text` as input
        claim_data = run_claim_extraction_on_statement(stmt["text"])
        
        # Add the id and speaker back in for the other modules
        claim_data["id"] = stmt["id"]
        claim_data["speaker"] = stmt["speaker"]
        
        extracted_claims.append(claim_data)
    
    print(f"Successfully extracted {len(extracted_claims)} claims.")
...
def run_debate_judge_pipeline(debate_transcript_path, topic):
    ...
    claims = run_module_2(statements)
